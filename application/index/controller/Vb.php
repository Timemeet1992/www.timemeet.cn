...
class Vb extends Controller
{
	public function vb()
	{
    	...
    	//查询分类
		$result = $cla->where('fatherid', 3)->where('goodsclass', '<>', 0)->where('isdisplay', 1)->select();
    	...
    	if($cid){
			...
			if(!empty($_GET['sort'])){
				if(($sort = $_GET['sort']) == 1){
			        ...
			        ->alias('c')
			        ->where('fatherid',3)->where('isfall', 1)
			        ->join('phone_goods g','c.cid = g.classify')
			        ->order('price', 'desc')
			        ->select();	
				}else{
					$this->error('未定义的排序', 'index/vb/vb');
				}
		...
		return $this->fetch('vb');
	}
	public function vbone()
	{
		$goods = new Goods();

		if(empty($_GET['gid'])){
			$this->error('您选择的商品不存在','index/vb/vb');
		}else{
			$gid = $_GET['gid'];
		...
		return $this->fetch('vbone');
	}

	public function dovbone()
	{
		$shop = new Shop();
		$goods = new Goods();
		...
		if($gid){
			...
			if(!$result){
				$this->error('商品不存在', url('index/vb/vbone', ['gid'=>$gid]));
			}
		}else{
		...
		if(!empty($_POST['action']) && $_POST['action'] == '立即购买'){
			...
			$res = $shop->save();
			if($res){
				$this->success('添加至购物车成功', url('index/vb/vbone', ['gid'=>$gid]));
			}else{
				$this->error('添加至购物车失败', url('index/vb/vbone', ['gid'=>$gid]));
			}
		}else{
			$this->error('请点击按钮进行提交', url('index/vb/vbone', ['gid'=>$gid]));
		}
	}
}